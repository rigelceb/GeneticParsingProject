...
public class SearchExtension {
    ...
    public static void main(String[] args) {
        ...
        boolean escape = !(scn.nextLine().equals("esc"));
        while(escape) {
            ...
            for(List<String> ls : parser.getDNA()) {
                if(rsid.equals(ls.get(0))) {
                    if(ls.get(3).contains("A")) {
                        System.out.println(rsid + " gene has allele: A");
                    } if(ls.get(3).contains("G")) {
                        System.out.println(rsid + " gene has allele: G");
                    } if(ls.get(3).contains("C")) {
                        System.out.println(rsid + " gene has allele: C");
                    } if(ls.get(3).contains("T")) {
                        System.out.println(rsid + " gene has allele: T");
                ...
                System.out.println("Type the word 'esc' to escape the search sequence and end the program. Otherwise, enter any sequence to search again.");
                scn.next();
                escape = !(scn.nextLine().equals("esc"));
        } scn.close();
        System.out.println("Thank you for utilising my Search Extension program!");
    }
}